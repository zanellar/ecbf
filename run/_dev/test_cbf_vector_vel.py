...
class clf_cbf_dubins():
    def __init__(self) -> None:
        ...
        x, y = sp.symbols('x y')  # define symbolic representation
        self.state = sp.Matrix([x, y])  # row vector

        e_x, e_y = sp.symbols('e_x, e_y,')
        self.target_state = sp.Matrix([e_x, e_y])  # symbols
    ...
    def init_system(self):
        ...
        self.f_symbolic = sp.Matrix([0,0])
        self.f = lambdify([self.state], self.f_symbolic)

        self.g_symbolic = sp.Matrix([[1, 0], [0, 1]])
        self.g = lambdify([self.state], self.g_symbolic)

        ########### CLF ###########

        self.clf_symbolic = (self.state[1] - self.target_state[1])**2 + (self.state[0] - self.target_state[0])**2 
        self.clf = lambdify([self.state, self.target_state], self.clf_symbolic)

        # Lie derivative of CLF w.r.t f and g  
        ...
        distance = (self.state[0] - self.obs_state[0])** 2 + (self.state[1] - self.obs_state[1]) ** 2 - (self.obs_radius + self.robot_radius) ** 2
        ...
        self.cbf_symbolic = distance
        self.cbf = lambdify([self.state, self.obs_state], self.cbf_symbolic)

        # Lie derivative of CBF w.r.t f and g 
    ...
    def render(self): 
        ...
        plt.grid('--')
        plt.show()
        self.ani.save(os.path.join(PLOTS_PATH,'animation.gif'), writer='pillow')
        plt.close()
